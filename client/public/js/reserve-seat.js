document.addEventListener("DOMContentLoaded", (event) => {
  ...
  const reservedSeats = JSON.parse(
    chairDiv.getAttribute("data-reserved-seats")
  );
    ...
    if (reservedSeats.includes(seatId)) {
      ...
      seat.addEventListener("click", () => {
        ...
        seatsCountElements.forEach((element) => {
          element.textContent = `${reservedCount} place${
            reservedCount > 1 ? "s" : ""
          } réservée${reservedCount > 1 ? "s" : ""}`;
        });

        totalReservedSeats.innerHTML = `${reservedCount} place${
          reservedCount > 1 ? "s" : ""
        }`;

        const totalPrice = reservedCount * pricePerSeat;
        totalPriceElement.textContent = `TOTAL A RÉGLER : ${totalPrice.toFixed(
          2
        )} €`;
