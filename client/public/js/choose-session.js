document.addEventListener("DOMContentLoaded", () => {
  ...
  const reservationButtonContainer = document.getElementById(
  ...
  function updateSessionAvailability(date) {
    ...
    fetch(
      `/reservation/get-sessions?date=${date}&cinemaId=${cinemaId}&filmId=${filmId}`
    )
          ...
          session.showtimes.forEach((showtime) => {
            ...
            sessionDiv.innerHTML = `
                <div class="flex flex-col space-y-3 items-center justify-center w-[60%]">
                  <h3 class="font-arvo font-bold text-lg text-blackOne">${startTime.toLocaleTimeString(
                    "fr-FR",
                    { hour: "2-digit", minute: "2-digit" }
                  )}</h3>
                  <p class="w-full text-center px-2 text-slate-300 font-bold font-arvo text-lg">${
                    showtime.quality
                  }</p>
                </div>
                <div class="flex items-center space-y-3 justify-center flex-col">
                  <p class="font-arvo text-nowrap font-light text-sm tracking-wide text-blackOne">(fin ${endTime.toLocaleTimeString(
                    "fr-FR",
                    { hour: "2-digit", minute: "2-digit" }
                  )})</p>
                  <div class="flex flex-col w-14 h-9 bg-goldOne items-center justify-center text-xs font-arvo font-bold text-blackOne py-4 ">
                    <p>Salle</p>
            ...
            sessionDiv.addEventListener("click", (e) => {
              ...
              document
                .querySelectorAll(".container.selected-day")
                .forEach((selectedDiv) => {
      ...
      .catch((error) => {
        ...
        availabilityContainer.innerHTML =
          "<p class='text-red-500 font-bold'>Aucune séance trouvée pour cette date.</p>";
