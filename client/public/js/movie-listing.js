document.addEventListener("DOMContentLoaded", () => {
  ...
  // Vérifiez s'il y a des films disponibles
  if (movieCards.length === 0) {
    reviewsButton.classList.add("hidden"); // Masquer le bouton "Noter" si aucune carte n'est disponible
  } else {
    reviewsButton.classList.remove("hidden"); // Afficher le bouton "Noter" s'il y a des cartes
  }
  ...
  function displayReservationInfo(info) {
    descriptionCard.querySelector(".movie-title").textContent =
      info.movie_title;
    descriptionCard.querySelector(".movie-date").textContent =
      formatDateToFrench(info.showtime_day);
    descriptionCard.querySelector(".movie-time").textContent =
      info.start_time.substring(0, 5);
    descriptionCard.querySelector(
      ".movie-duration"
    ).textContent = `Durée : ${info.movie_duration} min`;
    descriptionCard.querySelector(
      ".movie-end-time"
    ).textContent = `Fin prévue à ${info.end_time.substring(0, 5)}`;
    descriptionCard.querySelector(
      ".movie-poster"
    ).src = `/uploads/${info.movie_poster}`;
    descriptionCard.querySelector(".cinema-name").textContent =
      info.cinema_name;
    descriptionCard.querySelector(".cinema-location").textContent =
      info.cinema_location;
    descriptionCard.querySelector(".movie-seats").textContent = `${
      info.seats_reserved.length
    } Place${info.seats_reserved > 1 ? "s" : ""}`;
    descriptionCard.querySelector(".movie-quality").textContent = info.quality;
    descriptionCard.querySelector(
      ".movie-hall"
    ).textContent = `Salle ${info.room_name}`;
    descriptionCard.querySelector(".movie-accessibility").textContent =
      info.seat_accessibility ? "Accessible aux PMR" : "Non accessible aux PMR";
  }
