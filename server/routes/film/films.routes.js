...
filmsRoutes.get("/", async (req, res) => {
  try {
    ...
    const filterCurrentMovies = filterMovies(
      decMovies,
      genres,
      days,
      qualities
...
filmsRoutes.get("/disponibiliter/:id", async (req, res) => {
  ...
  try {
    ...
    const frenchCinemas = cinemas.filter(
      (cinema) => cinema.country.toLowerCase() === "france"
    );
    const cinemaShowtimes = frenchCinemas.map((cinema) => {
      const cinemaShowtime = showtimes.find(
        (show) => show.cinema_id === cinema.cinema_id
      );
      return {
        ...cinema,
