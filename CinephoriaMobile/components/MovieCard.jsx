...
const MovieCard = ({ movie }) => { // + Ajout de la prop `movie`
  ...
  // Handler for pressing the QR code button
  const handlePress = async () => {
    setLoading(true);
    try {
      // Generate the URL with the token and reservation ID
      const qrCodeUrl = `http://192.168.1.21:3000/ticket/${movie.reservation_id}?token=${movie.token}`;
      console.log(qrCodeUrl)
      setQrCodeValue(qrCodeUrl); // Update state with the QR code URL
    } catch (error) {
      console.error('Error generating QR code URL:', error);
    } finally {
      setLoading(false);
      setIsModalVisible(true); // Show the modal after generating the URL
    }
  };
      ...
      <View className="flex-row items-start">
        <View className="justify-center items-center flex-row flex-1">
          <View className="w-full h-60">
            <Image source={{ uri: `http://192.168.1.21:3000/uploads/${movie.movie_poster}` }} className="w-full h-full" resizeMode="cover" />
          </View>
        </View>
